...
const taskProgram = {
    ...
    add(obj, redoundo = 0) {
        if (this.checkName(obj)) {
            let checkedTagObj = this.checkTag(obj);
            let newTodo = new Task(checkedTagObj)
    ...
    checkName(obj) {
        if (typeof (obj) !== 'object') {
            console.log(' {name : ""  , tag :  "" } 의형태로 입력해주세요');
            return false;
    ...
    update(obj, undoredo = 0) {
        if (this.checkParametertype(obj)) {
            ...
            if (this.checkId(transobj) && this.checkState(transobj) && this.checkupdateState(transobj)) {
    ...
    checkupdateState(obj){
        let samestate = this.taskArray.filter(v => v.id === obj.id && v.state === obj.nextstatus)
        if(samestate.length !== 0){
            console.log(`${obj.id}번의 상태는 이미 ${obj.nextstatus} 입니다`) 
            return false;
        }
        return true;
    },
...
taskProgram.update({ id: 0, nextstatus: '      todo' });
